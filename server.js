...
app.post('/api/start-auth', async (req, res) => {
    ...
    try {
        ...
        // Use the correct method for sending code
...
app.post('/api/verify-code', async (req, res) => {
    ...
    try {
        ...
        let user;
        try {
            // Use the correct signIn method - pass parameters directly
            user = await client.signIn(phone, {
                phoneCode: code,
                phoneCodeHash: phoneCodeHash,
            });
            console.log('✅ Sign in successful with verification code');
        } catch (signInError) {
            console.log('⚠️ First sign in attempt failed:', signInError.message);
            ...
            if (signInError.error_message === 'SESSION_PASSWORD_NEEDED') {
                ...
                // Handle 2FA with correct method
            ...
            } else if (signInError.error_message === 'PHONE_CODE_INVALID' || signInError.message.includes('PHONE_CODE_INVALID')) {
        ...
        } else if (error.error_message === 'PASSWORD_HASH_INVALID') {
            errorMessage = '❌ Invalid 2FA password. Please check your password and try again.';
        } else if (error.message.includes('FLOOD_WAIT')) {
            const waitTime = error.message.split('FLOOD_WAIT_')[1] || 'a few';
